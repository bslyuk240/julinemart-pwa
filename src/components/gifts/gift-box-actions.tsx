...
export default function GiftBoxActions({ box }: Props) {
  const { isFavorite, toggleFavorite } = useGiftBoxFavorites();
  const [showShareMenu, setShowShareMenu] = useState(false);
  const favorited = isFavorite(box.id);
  ...
  const handleWhatsAppShare = () => {
    const text = encodeURIComponent(`Check out this gift box: ${box.name} - ${formatPrice(box.list_price)}\n${window.location.href}`);
    window.open(`https://wa.me/?text=${text}`, '_blank');
  };

  const handleTwitterShare = () => {
    const text = encodeURIComponent(`Check out the ${box.name} gift box on JulineMart!`);
    const url = encodeURIComponent(window.location.href);
